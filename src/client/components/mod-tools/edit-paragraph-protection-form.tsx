...
function EditParagraphProtectionForm({ onToggleSlider, isSlided, showToast, access_token, user, paragraphProtection, handleAction }) {

  const [max, setMax] = useState(paragraphProtection.max);
  const [responseStatus, setResponseStatus] = useState(paragraphProtection.responseStatus);
  const [response, setResponse] = useState(paragraphProtection.response);
  const [punishmentTime, setPunishmentTime] = useState(paragraphProtection.punishmentTime);
  const [punishment, setPunishment] = useState(paragraphProtection.punishment);
  const [userLevel, setUserLevel] = useState(paragraphProtection.userLevel || "moderator");

  useEffect(() => {
    setMax(paragraphProtection.max);
    setResponseStatus(paragraphProtection.responseStatus);
    setResponse(paragraphProtection.response);
    setPunishmentTime(paragraphProtection.punishmentTime);
    setPunishment(paragraphProtection.punishment);
    setUserLevel(paragraphProtection.userLevel || "moderator");
  }, [paragraphProtection]);

  const handleEditParagraphProtection = async (e) => {
    try {
      e.preventDefault();
      const { data, status } = await axios.put(`/api/mod-tools/paragraph-protection/${paragraphProtection.id}`, {
        max: max,
        responseStatus: responseStatus,
        response: response,
        punishmentTime: punishmentTime,
        punishmentType: punishment,
        status: paragraphProtection.status,
        userLevel: userLevel,
        userId: user.id,
      }, {
        headers: {
      ...
      handleAction(updatedParagraphProtection, 'update');
    ...
    <>
      ...
      <div className={ isSlided ? "flex flex-col fixed min-h-screen max-h-screen max-w-screen-md w-full top-0 right-0 z-30 border-l border-gray-800 bg-gray-900 text-white transform transition duration-500 ease-in-out shadow-2xl" : "flex flex-col fixed min-h-screen max-h-screen max-w-screen-md w-full top-0 right-0 z-20 border-l border-gray-800 bg-gray-900 text-white transform transition duration-500 ease-in-out translate-x-full" }>
        <div className="flex justify-between pb-5 pt-6 px-6">
            <div className="text-lg font-semibold">Paragraph Protection</div>
        ...
        <div className="flex flex-grow flex-col h-full overflow-auto pb-14 px-6">
          <form onSubmit={handleEditParagraphProtection}>
            ...
            <div className="grid grid-cols-1 md:grid-cols-1 gap-6 mb-6">
              <div className="relative w-full">
                <label className="relative text-sm font-semibold" htmlFor="">
                  Max
                  <QuestionTooltip content={"Maximum amount of characters permited in a message."}/>
                </label>
...
export default EditParagraphProtectionForm;