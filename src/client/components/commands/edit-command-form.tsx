...
function EditCommandForm({ onToggleSlider, isSlided, showToast, handleCommandAction, access_token, user, editableCommand }) {

  const [command, setCommand] = useState(editableCommand.command || "");
  const [description, setDescription] = useState(editableCommand.description || "");
  const [response, setResponse] = useState(editableCommand.response ? editableCommand.response.split("@@") : [""]);
  const [globalCooldown, setGlobalCooldown] = useState(editableCommand.globalCooldown || 0);
  const [userCooldown, setUserCooldown] = useState(editableCommand.userCooldown || 0);
  const [commandType, setCommandType] = useState(editableCommand.commandType || "simple");
  const [userLevel, setUserLevel] = useState(editableCommand.userLevel || "all");
  const [aliases, setAliases] = useState(editableCommand.aliases && editableCommand.aliases.length > 0 ? editableCommand.aliases.split(",") : []);
  const [keywords, setKeywords] = useState(editableCommand.keywords && editableCommand.keywords.length > 0 ? editableCommand.keywords.split(",") : []);
  
  const [alias, setAlias] = useState("");
  const [keyword, setKeyword] = useState("");

  useEffect(() => {
    setCommand(editableCommand.command || "");
    setDescription(editableCommand.description || "");
    setResponse(editableCommand.response ? editableCommand.response.split("@@") : [""]);
    setGlobalCooldown(editableCommand.globalCooldown || 0);
    setUserCooldown(editableCommand.userCooldown || 0);
    setCommandType(editableCommand.commandType || "simple");
    setUserLevel(editableCommand.userLevel || "all");
    setAliases(editableCommand.aliases && editableCommand.aliases.length > 0 ? editableCommand.aliases.split(",") : []);
    setKeywords(editableCommand.keywords && editableCommand.keywords.length > 0 ? editableCommand.keywords.split(",") : []);
  }, [editableCommand]);

  const handleEditCommand = async (e) => {
    try {
      e.preventDefault();
      const { data, status } = await axios.put(`/api/custom-command/${editableCommand.id}`, {
        command: command,
      ...
      if(!(status === 204 || status === 201 || status === 200)) {
        throw new Error("Unable to Edit command");
      }

      const updatedCommand = data;
      handleCommandAction(updatedCommand, 'update');
      showToast('success', `Command ${updatedCommand.command} successfully updated`);
      handleSlider(e);
    } catch(error) {
      if(!error.message) {
    ...
    <>
      ...
      <div className={ isSlided ? "flex flex-col fixed min-h-screen max-h-screen max-w-screen-md w-full top-0 right-0 z-30 border-l border-gray-800 bg-gray-900 text-white transform transition duration-500 ease-in-out shadow-2xl" : "flex flex-col fixed min-h-screen max-h-screen max-w-screen-md w-full top-0 right-0 z-20 border-l border-gray-800 bg-gray-900 text-white transform transition duration-500 ease-in-out translate-x-full" }>
        <div className="flex justify-between pb-5 pt-6 px-6">
            <div className="text-lg font-semibold">Edit Command</div>
                ...
                <div className="grid grid-cols-1 gap-2">
                {
                  ...
                  }
                  {
                    commandType === "simple" ?
                      ""
                      :
                      <button onClick={addResponse} className="mt-2 pt-1 px-3 border border-gray-800 text-blue-500 rounded-lg rounded-br-lg hover:bg-gray-800 transition-all duration-500 font-semibold">
